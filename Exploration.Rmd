...
The first question asked of any data set is always whether we have missing data and their occurrence. Therefore, first of all, we check the completeness of the observations within our dataset.
...
```{r bwght}
ggplot(data = data, aes(x = data$bwght)) + 
      geom_histogram(bins = 50, fill="dodgerblue1", colour = "black") +
      ggtitle('Birthweight of Infants') + 
      xlab("Baby Birthweight (in grams)") + ylab('Count') + 
...
```{r monpre}
ggplot(data = data, aes(x = data$monpre)) + 
        geom_histogram(binwidth = 1, fill="dodgerblue1", 
                       colour = "black", boundary = -0.5) +
...
```{r npvis}
ggplot(data = data, aes(x = data$npvis)) + 
        geom_histogram(binwidth = 1, fill="dodgerblue1", 
                       colour = "black", boundary = -0.5, bins = 30) +
        ggtitle('Number of Prenatal Care Visits') + 
        xlab("Prenatal Care Visits") + ylab('Count') + 
        theme(axis.text.x = element_text(face = "bold"), 
              axis.text.y = element_text(face = "bold"), 
              plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
```
...
```{r}
ggplot(data = data, aes(x =data$monpre , y = data$npvis)) + geom_point() + 
      geom_smooth(method = "loess") + geom_jitter(width = 0.2, height = 0.1) + 
      ggtitle("Month Prenatal Care Began vs. Total Number of Prenatal Visits") + 
      xlab("Month Prenatal Care Began") + ylab("Total Number of Prenatal Visits") + 
      theme(axis.text.x = element_text(face = "bold"), 
            axis.text.y = element_text(face = "bold"), 
            plot.title = element_text(size = 14, face = "bold", hjust = 0.5)) + 
...
```{r meduc}
ggplot(data = data, aes(x = meduc)) + 
      geom_histogram(binwidth = 1, fill="dodgerblue1", 
                     colour = "black", boundary = -0.5, bins = 30) + 
      ggtitle("Education Level of Mothers") + 
      xlab("Education (Years)") + ylab("Count") + 
      theme(axis.text.x = element_text(face = "bold"), 
            axis.text.y = element_text(face = "bold"), 
            plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
```
...
ggplot(data = data, aes(x = cigs)) + 
  geom_histogram(binwidth = 1, fill="dodgerblue1", colour = "black", boundary = -0.5) +
...
A final step in the exploratory data analysis involves inspecting the bivariate relationships to determine whether there are nonlinear relationships present. This step will also help us develop an intuition about what results to expect and possibly troubleshoot outlier issues that may arise during the modeling procees. We only include plots of our key prenatal independent variables and the dependent birthweight variable.

The bivariate plot of number of prenatal visits and infant birthweight shows that some amount of visits do seem to be related to an increase in birthweight. However, this positive trend levels off around visit 12. There appear to be some potential outliers towards the right end of visit axis; these seem to result in a wider variance for the loess line. We will want to check whether these points are influential or not when we do our regression analysis.

```{r}
ggplot(data = data, aes(x = data$npvis, y = data$bwght)) + geom_point() + 
      geom_smooth(method = "loess") + geom_jitter(width = 0.2, height = 0.1) + 
      ggtitle("Total Number of Prenatal Visits vs. Infant Birthweight") + 
      xlab("Total Number of Prenatal Visits") + ylab("Birthweight (grams)") + 
      theme(axis.text.x = element_text(face = "bold"), 
            axis.text.y = element_text(face = "bold"), 
            plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
```

When we look at the month prenatal care began vs. birthweight, we are surprised to see a nearly flat loess line. That is, when prenatal care starts is unrelated to the birthweight of a baby. Intuitively, we would expect mother's who start early to be more informed and more conscientious than other mothers. This does not appear to be the case. Therefore, we would not expect this variable to carry much weight in the regression analysis.

```{r}
ggplot(data = data, aes(x = data$monpre, y = data$bwght)) + geom_point() + 
      geom_smooth(method = "loess") + geom_jitter(width = 0.2, height = 0.1) + 
      ggtitle("Month Prenatal Care Began vs. Infant Birthweight") + 
      xlab("Start Month of Prenatal Care") + ylab("Birthweight (grams)") + 
      theme(axis.text.x = element_text(face = "bold"), 
            axis.text.y = element_text(face = "bold"), 
            plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
```
...
```{r model3}
model3 = lm(bwght ~ monpre + npvis + smoker + male + mage + mheduc + mceduc + 
            fheduc + fceduc + fage, data = data)
plot(model3, which = 1)
```
...
linearHypothesis(model3, c("fheduc = 0", "fceduc = 0", "fage = 0", "mage = 0", 
                           "mheduc= 0", "mceduc = 0"), vcov = vcovHC)
```
...
stargazer(model1, model2, model3, type = "latex", report = "vcsp*", header = FALSE,
          ...
          p.auto = TRUE, title = "Prenatal Care Influence In Birthweight",
...
When we add in our control variables in the second model, we witness over a doubling in $R^2$, meaning that this model now accounts for more of the variation in the data than the model without the control variables. Although this is expected, as $R^2$ always increases when adding new variables, other measurements used that penalize the model for added complexity such as the Adjusted $R^2$ and Akaike Information Criterion, also displayed in table 11, both demonstrate improvement in model fit.

Two of the new variables introduced in the second model are statistically significant at the .001 level: `smoker` and `male`. Both cases also display practical significance, with the coefficient values indicating that a male baby is likely to be 73 grams heavier when everything else is held constant and that a smoker mother is likely to have a baby 205 grams lighter than a non-smoking mother on average, ceteris paribus. The addition of new variables also made the initial month for prenatal care statistically significant at a 95% confidence level. This indicates that mothers would be likely to have a baby 24 grams heavier for each month closer to conception when they begin their prenatal care, which goes against our preconceived notion that longer prenatal care would be more likely to result in healthier babies.
...
By verifying the third model, we can see that all previous significant variables remained significant, except for the month prenatal care began, and that the father age is highly statistically significant. Its coefficient indicates that for each additional year in the father age, holding everything else constant, the baby is likely to present an approximate 9.4 grams increase in weight. While this may seem a small value, a 40-year-old father would, on average, have a baby that is 94 grams heavier than a 30-year-old father. As explained before, it goes against the logic that older fathers would give birth to healthier babies, so this could be a potential confounding effect that we should be aware of that is probably related to family income, which we would expect to be higher for older fathers, or the capability of older fathers to provide for mothers while they are dedicated to their pregnancy.

# Fun Model Selection Comparison

We want to see how our models compare to a software package designed to maximize adjusted $R^2$ by searching over all possible subsets of the explanatory variables. The package in R is Leaps and we used it to conduct an exhaustive search, although no interaction terms or higher order polynomials were incluced. The results are illuminating. Including the number of prenatal visits squared in the model seems to increase the adjusted $R^2$; this would mean that the relationship between the number of visits and the weight of an infant is nonlinear. And surprisingly, race appears to matter in some cases, specifically with the father. We would not have expected this kind of result and it may suggest that these variables are proxies for something else. Regardless, it would appear we left some variation that could have been accounted for on the table in favor of a more parsimonious model.
```{r regsubsets, echo = F}
regsubsets_out <- regsubsets(bwght ~ mage + monpre + npvis + fage  + smoker + drink + male + mwhte + mblck + fwhte + fblck + npvissq + mheduc + mceduc + fheduc + fceduc, data = data, nbest = 1, nvmax = NULL, force.in = NULL, force.out = NULL, method = "exhaustive")
...
In one final blow to causality and any interpretation of these analyses, many different models were run without concern for issues of multiple hypothesis testing. No correction was used to ensure that the p-values calculated were corrected for the multiple hypotheses, so there is a higher likelihood that the statistically significant results we found are spurious.

# Conclusion

Even though we cannot make causal claims from our analyses, the results are nonetheless important for informing policy. We can say that prenatal visits and when a mother starts those visits are associated with the weight of an infant. Policy makers would want to encourage newly expecting mothers to start their prenatal visits early and to go often. Although the likely impact on the health of an infant may not be exceedingly large, in the worst case scenarios the visits may be a good check against unforeseen issues that may not be captured in the data. 

It is clear from our analyses that smoking has a negative effect on the health of an infant. Education on this matter should start early and be reinforced over time. Due to the small representativeness in the sample, we cannot make any affirmation regarding drinking during pregnancy. 

Other demographic variables play a role in the health of an infant, but deciphering their meaning is not so straightforward. For instance, we found that the father's age has a positive influence in the baby's birthweight, but it is unlikely that this is a direct relationship especially because it goes against the aforementioned logic. We believe it is most likely that older fathers might have waited until achieving some modicum of financial stability or of professional success before having a child, and therefore were able to provide for the family while the mother is dedicated to her pregnancy and prenatal care. It is also possible that older fathers already had other children, and therefore are more informed regarding pregnancy and prenatal care best practices.

In conclusion, many of the variables that could be integral to understanding the larger picture with regard to an infant's health are now being captured in the error term. The omission of such important variables may be causing omitted variable bias in the previously regressed models and hence causing the coefficients to also be highly biased. While our report does appear to give clear guidance on the matters of smoking and prenatal care visits, these conclusions must be taken with a grain of salt due to the organization of the underlying study.
