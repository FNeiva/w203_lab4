...
which(is.na(data["cigs"]) == T & is.na(data[,"drink"]) == T ) # 107 observations missing both drinking and cigs, we should see whether these people in a separate regression have different baby weights?
...
data[c(which(apply(missing_data,1,mean)>.2)),] # same as below, points with most missing values
which(missing_data[,"monpre"] == T & missing_data[,"npvis"] == T ) # These are teh points that have the most missing values
which(data[,"monpre"] == 0 & data[,"npvis"] == 0 ) # 5 points with 0,0 on monpre and npvis 

# There are four data points that are missing values for mpvis and monpre
```
...
```{r}
summary(data[data$male ==1,]$bwght) # Male
summary(data[data$male ==0,]$bwght) # Female
```
...
Zero mean conditional: We see a negative trending line in our residuals that starts above zero and ends negative. Ideally, we would want a horizontal line at 0. The deviations from zero happen towards the fringes of the fitted values, suggesting that high leverage points may be affecting the fit of the model. Within the most dense portion of the data, we do see that the zero mean conditional is satisfied.
Homoskedasticity
Normality of Errors
...
stargazer(model1, model2, model3, type = "latex", report = "vc", header = FALSE,
          ...
          add.lines = list(c("AIC", 
                             round(AIC(model),digits=3), 
                             round(AIC(model_rev), digits=3))))
```
