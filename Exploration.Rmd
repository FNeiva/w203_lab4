...
```{r} 
which(is.na(data["cigs"]) == T & is.na(data[,"drink"]) == T )
```
There is also a very high percentage of missing data from the same people on the smoking and drinking quesiton. 
```{r}
...
Finally, the omaps and fmaps variables have the exact same number of missing data. If this happens on the same observation, then we might have hospital bias, as some hospitals might not do these measurements. 
